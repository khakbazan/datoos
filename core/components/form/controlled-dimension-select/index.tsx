...
import { useGetDimensionList } from "@/models/products";

export const ControlledDimensionSelect: React.FC<
...
> = ({ name }) => {
  ...
  const { data: dimensions, isFetching } = useGetDimensionList({
    params: {
      name: searchValue,
      page_size: 15,
      page_number: 0,
                  ...
                  {dimensions?.data?.length ? (
                    dimensions?.data?.map((dimension) => {
                      return (
                        ...
                        >
                          {dimension?.name ?? dimension?.title ?? "-"}
                        </Radio>
                      );
                    })
                  ) : (
